...
#include <arpa/inet.h>   // ★ 加上这个！inet_addr()用到
#include <unistd.h>      // ★ 加上这个！close()用到
#include <errno.h>       // ★ 可选：如果后面需要用errno的话
...
static void deliver_content(FILE *fout, const char *filename, struct chmFile *file)
{
    ...
    unsigned long long swath, offset;  // 使用 unsigned long long 类型
